...
class MIP_IF():
    ...
    def KCluster_ind_Gurobi_function(self, X, K, label, UB, M, T, reg):
        """Fit a K-cluster LDS model with Gurobi."""
        import gurobipy as gp
        from gurobipy import GRB

        e = gp.Env()
        e.setParam('TimeLimit', 6*60*60)
        model = gp.Model(env=e)
        T -= 1
        I = len(X)
        ...
        f = model.addVars(K, M, T, name="f", vtype='C')
        phi = model.addVars(K, UB, (T + 1), name="phi", vtype='C')
        G = model.addVars(K, UB, UB, name="G", vtype='C')
        F = model.addVars(K, M, UB, name="F", vtype='C')
        q = model.addVars(K, UB, T, name="q", vtype='C')
        p = model.addVars(K, M, T, name="p", vtype='C')
        quatr = model.addVars(K, M, (T + 1), name="quatr", vtype='C')
        quatr_hidden = model.addVars(K, UB, T, name="quatr_hidden", vtype='C')
        z_squared = model.addVars(K, I, T, M, name="z_squared", vtype='C')

        obj = gp.quicksum(
            L[i, k] * z_squared[k, i, t, m]
            for k in range(K)
            for i in range(I)
            for t in range(T)
            for m in range(M)
        ...
        model.addConstrs(
            (
                z_squared[k, i, t, m]
                >= (X[i, t, m] - f[k, m, t]) ** 2 - (1 - L[i, k]) * M_big
            )
            for k in range(K)
            for i in range(I)
            for t in range(T)
